...
train_features = ["duration", "protocol_type", "src_bytes", "dst_bytes", "count",
                  "srv_count", "is_guest_login", "root_shell", "num_failed_logins", "" "class"]
...
def CreateBatch_GAN(x, batch_size):
    ...
    batch_x = [x[batch_size * i: (i + 1) * batch_size, :]
...
def CreateBatch_BlackBox(data, label, batch_size):
    ...
    batch_data = [data[batch_size * i: (i + 1) * batch_size, :].tolist()
                  for i in range(len(data) // batch_size)]
    batch_label = [label[batch_size * i: (i + 1) * batch_size].tolist()
                   for i in range(len(data) // batch_size)]
...
def Preprocess_GAN(train):
    ...
    numeric_columns = list(train.select_dtypes(
...
def Preprocess_BlackBox(train, test):
    ...
    for t in trash:
        del test[t]
    numeric_columns = list(test.select_dtypes(
        include=['int', "float"]).columns)
    for c in numeric_columns:
        max_ = test[c].max()
        min_ = test[c].min()
    ...
    # return data
    train_data, train_label = np.array(
        train[train.columns[train.columns != "class"]]), np.array(train["class"])
    test_data, test_label = np.array(
        test[test.columns[test.columns != "class"]]), np.array(test["class"])
